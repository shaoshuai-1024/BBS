...
def register(request):
    ...
    if request.method == 'POST':
        ...
        if form_obj.is_valid():
            # print(form_obj.cleaned_data)  # {'username': 'shuai', 'password': '123', 'confirm_password': '123', 'email': '2895788257@qq.com'}
            clean_data = form_obj.cleaned_data  # 将校验通过的数据字典赋值给一个变量
            # 将字典里面的confirm_password建值对删除
            clean_data.pop('confirm_password')
            # 用户头像
            file_obj = request.FILES.get('avatar')
            """针对用户头像一定要判断是否有值 不能直接添加到字典里面去"""
            if file_obj:
                clean_data['avatar'] = file_obj
            # 直接操作数据库保存数据
            models.UserInfo.objects.create_user(**clean_data) # create_user:自动给密码加密
            back_dic['url'] = '/login/'
        else:
            back_dic['code'] = 2000
...
"""
内存管理器模块
BytesIO：临时帮你储存数据 返回的时候数据是二进制
StringIO：临时帮你储存数据 返回的时候数据是字符串
"""
import random
def get_random():
...
def get_code(request):
    ...
    img_obj = Image.new()
    括号内三个参数 模式 尺寸 颜色
    get_random()： 多个数没有说明类型，python自动会用元组括主  ( , , )
    """
    ...
    for i in range(5):
        ...
        code += tmp
    print(code)
    # 随机验证码在登录的视图函数里面需要用到 要对比 所以要找地方存起来并且其他视图函数也能拿到
    request.session['code'] = code   # 把验证码存到session里面
    io_obj = BytesIO()
...
def site(request,username,**kwargs):
    ...
    if kwargs:
        ...
        if condition == 'category':
            ...
            year,month = param.split('-')  # 2022-07  [2022,07]
        ...
        if request.user.is_authenticated():
            ...
            if not article_obj.blog.userinfo == request.user:
                ...
                if not is_click:
                    ...
                    back_dic['code'] = 1001
                    back_dic['msg'] = '你已经点过了，不能再点了'  # 这里你可以做的更加详细 提示用户到底点了赞还是点了踩
            else:
                back_dic['code'] = 1002
